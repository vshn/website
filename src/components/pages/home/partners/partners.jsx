...
const Partners = ({ items, itemButtonText }) => {
  ...
  const [activeItemIndex, startAnimation, restartAnimation] =
    useAutoChangeableIndex(items.length, { interval: ITEM_CHANGE_INTERVAL });
      ...
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            {isActive ? (
              <ProgressCircle
                className={cx("tab-progress-circle")}
                animate={isActive}
              >
                {number}
              </ProgressCircle>
