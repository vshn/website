...
const Team = ({
  data: { wpPage: data, allWpTeamMember, allWpTeam },
  pageContext: { locale, pageUrls, menus, globalFields },
}) => {
  const breadcrumbs = [t[locale].breadcrumbs.about];
  const getAllTeamRoles = allWpTeamMember.items.map(
    (item) => item.acf.jobTitle
  );
  ...
  query ($id: String!, $locale: String!) {
    ...
    allWpTeamMember(
      ...
      items: nodes {
        ...
        acf {
          image {
            gatsbyImage(width: 300, height: 200, fit: COVER, placeholder: NONE)
          }
          jobTitle
          jobPosition
          socialLinks {
